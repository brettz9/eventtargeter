...
/* eslint-disable no-shadow -- Polyfill */
    ...
    function DOMException (msg, name) { // No need for `toString` as same as for `Error`
        ...
        Object.defineProperty(err, 'name', {value: name, writable: true, configurable: true});
        return err;
    }
    /* c8 ignore stop */
    : DOMException;
                    ...
                    : (Reflect.has(_ev, prop)
                        ...
                        : (
                            ['bubbles', 'cancelable', 'composed'].includes(prop)
                                ? false
                                /* c8 ignore next -- Every prop this is ever called with ('type'/'bubbles'/'cancelable'/'timeStamp'/'composed'/'detail') always ends up set on `_evCfg` or `_ev` by the time it's read, so this is unreachable */
                                : undefined
                        ));
            }
        };
...
function getIsTrusted () {
    ...
    return Boolean(
        Object.hasOwn(_evCfg, 'isTrusted')
            /* c8 ignore next -- Nothing currently sets `_evCfg.isTrusted` (see the commented-out assignment above); kept for when/if it's revived */
            ? _evCfg.isTrusted
...
[
    ...
    'eventPhase', 'defaultPrevented', 'bubbles', 'cancelable', 'timeStamp',
    'composed'
].forEach((prop) => {
    const get = function () {
        throw new TypeError('Illegal invocation');
...
});
{
    ...
    Object.defineProperty(get, 'name', {value: 'get returnValue', configurable: true});
    Object.defineProperty(set, 'name', {value: 'set returnValue', configurable: true});
...

{
    ...
    Object.defineProperty(get, 'name', {value: 'get detail', configurable: true});
...
function copyEvent (e) {
    ...
    if ('detail' in e) {
        // @ts-expect-error Casting doesn't work
        return new ShimCustomEvent(type, {bubbles, cancelable, detail}, e);
    }
    // @ts-expect-error Casting doesn't work
...
Object.assign(EventTarget.prototype, ['Early', '', 'Late', 'Default'].reduce(function (
    ...
    ['add', 'remove', 'has'].forEach(function (method) {
        ...
        obj[mainMethod] = function (type, listener) {
            ...
            try {
                ...
                if (listener && typeof listener === 'object' &&
                    'handleEvent' in listener && listener.handleEvent.bind) {
        ...
        Object.defineProperty(obj[mainMethod], 'name', {value: mainMethod, configurable: true});
    });
    return obj;
}, {}));
...
Object.assign(EventTarget.prototype, {
    ...
    _dispatchEvent (e, setTarget) {
        ...
        if (_evCfg) {
            ...
            (this._extraProperties).forEach((prop) => {
                if (Reflect.has(e, prop)) {
                    /** @type {PropertyDescriptorMap} */ (eventCopy)[prop] =
                        /** @type {PropertyDescriptorMap} */ (e)[prop]; // Todo: Put internal to `ShimEvent`?
                }
            });
        }
        const {type: rawType} = eventCopy;
        ...
        function invokeDefaults () {
            ...
            if (!eventCopy.defaultPrevented || !cfg.cancelable) { // 2nd check should be redundant
                cfg.eventPhase = phases.AT_TARGET; // Temporarily set before we invoke default listeners
                getTarget().invokeCurrentListeners(
                    /** @type {AllListeners} */ (getTarget()._defaultListeners), eventCopy, type
                );
            }
            finishEventDispatch();
        }
        const continueEventDispatch = () => {
            ...
            if (!cfg._stopPropagation) {
                ...
                getTarget().invokeCurrentListeners(
                    /** @type {AllListeners} */ (getTarget()._lateListeners), eventCopy, type
                );
            }
            finishEventDispatch();
        ...
        if (typeof window === 'undefined' || typeof ErrorEvent === 'undefined' || (
            ...
            triggerGlobalErrorEvent = () => {
                ...
                const errEv = new ErrorEvent('error', {
                    ...
                    filename: /** @type {Error & {fileName: string}} */ (
                        error
                    ).fileName || '',
                    lineno: /** @type {Error & {lineNumber: Integer}} */ (
                        error
                    ).lineNumber || 0,
                    colno: /** @type {Error & {columnNumber: Integer}} */ (
                        error
                    ).columnNumber || 0
                });
